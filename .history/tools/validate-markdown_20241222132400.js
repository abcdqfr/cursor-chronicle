...
function autoFixMarkdown(content) {
  ...
  lines = lines.map((line, i) => {
    ...
    if (line.match(/^```\s*$/)) {
      ...
      const langHint = prevLines.find(l => l.match(/\b(javascript|python|typescript|html|css|json|yaml|bash|sh)\b/i));
      const lang = langHint ? langHint.match(/\b(javascript|python|typescript|html|css|json|yaml|bash|sh)\b/i)[1].toLowerCase() : "text";
      line = `\`\`\`${lang}`;
    }
    if (line.match(/^```/)) {
