...
export function convertBaseContextToUri(context: BaseContext): vscode.Uri {
    return vscode.Uri.parse(`${context.scheme || 'file'}://${context.authority || ''}${context.path || ''}${context.query ? `?${context.query}` : ''}${context.fragment ? `#${context.fragment}` : ''}`);
} 