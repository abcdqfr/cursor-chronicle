...
class DocumentContext {
  ...
  addHeading(level, text, lineNumber) {
    ...
    if (this.seenHeadings.has(normalizedText)) {
      ...
      this.seenHeadings.set(normalizedText, { count: 1, firstLine: lineNumber });
    }
  ...
  validateStructure() {
    ...
    this.headings.forEach(({ level, text, lineNumber }) => {
      ...
      if (level === 1) {
        if (hasTopLevel) {
          errors.push(`Multiple top-level headings in the same document [${lineNumber}:0]`);
        }
        hasTopLevel = true;
      }
    ...
    this.seenHeadings.forEach((data, text) => {
      if (data.count > 1) {
        errors.push(`Duplicate heading "${text}" first used at line ${data.firstLine}`);
      }
    });
  ...
  inferCodeBlockLanguage(content, surroundingLines) {
    ...
    const patterns = {
      ...
      typescript: /\b(interface|type|enum|namespace|readonly|private|public|protected)\b/,
...
async function validateMarkdown(content, configPath) {
  const defaultConfigPath = path.join(__dirname, "../../config/linting/markdownlint.json");
...
function autoFixMarkdown(content) {
  ...
  lines = lines.map((line, i) => {
    ...
    if (line.match(/^```\s*$/)) {
      ...
      // Infer language
      const lang = context.inferCodeBlockLanguage(codeContent, surroundingLines);
      line = `\`\`\`${lang}`;
    }
    ...
    if (line.match(/^```/)) {
      ...
      if (line.match(/^```\w*$/) && i < lines.length - 1 && lines[i + 1] !== "") {
        lines.splice(i + 1, 0, "");
      }
    }
